-- Adds `slug` to get_friend_equipped_items so the client can resolve item icons
-- via storage `item-icons/<slug>.png`. The return shape changes, so we drop and
-- recreate (CREATE OR REPLACE cannot alter RETURNS TABLE columns).

DROP FUNCTION IF EXISTS public.get_friend_equipped_items(UUID);

CREATE FUNCTION public.get_friend_equipped_items(p_friend_id UUID)
RETURNS TABLE (
  item_id UUID,
  slug TEXT,
  name TEXT,
